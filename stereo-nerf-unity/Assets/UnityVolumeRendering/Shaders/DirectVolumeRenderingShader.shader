Shader "VolumeRendering/DirectVolumeRenderingShader"
{
    ...
    SubShader
    {
        ...
        {
            CGPROGRAM
            #pragma multi_compile MODE_DVR MODE_DVR_RGBA MODE_MIP MODE_SURF
...
#define EPSILON 0.00001f
                ...
                //    {
                //        float4 sampledColor = tex3D(_DataTextRGBA, samplePosition + float3(0.5f, 0.5f, 0.5f));
                //        sampledColor.a *= 0.02;
                //        col = BlendUnder(col, sampledColor);
                //        samplePosition += rayDirection * 0.01; // step_size = 0.01
                //    }
                //}
                ...
                for (int iStep = 0; iStep < raymarchInfo.numSteps; iStep++)
                {
                    ...
                    if (density > _MinVal && density < _MaxVal)
                    {
                        ...
                        //col.rgb = calculateLighting(col.rgb, normal, -ray.direction, -ray.direction, 0.15);
                        //col.a = 1.0f;
                        //col = tex3D(_DataTextRGBA, currPos);
                        col.rgb = getRGBAColor(currPos);
                        col.a = density;
                        break;
                    }
                    else {
