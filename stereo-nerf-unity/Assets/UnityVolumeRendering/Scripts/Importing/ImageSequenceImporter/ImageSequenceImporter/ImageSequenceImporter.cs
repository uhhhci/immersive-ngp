...
namespace UnityVolumeRendering
{
    ...
    public class ImageSequenceImporter : IImageSequenceImporter
    {
        ...
        public VolumeDataset ImportSeries(IImageSequenceSeries series)
        {
            ...
            Color[] data_rgb = FillRGBStack(dimensions, imagePaths);
            VolumeDataset dataset = FillVolumeDatasetRGB(data, data_rgb,  dimensions);
        ...
        /// <param name="paths">The set of image paths comprising the volume.</param>
        /// <returns>The set of sequential values for the volume.</returns>
        private Color[] FillRGBStack(Vector3Int dimensions, List<string> paths)
        {
            //var data = new List<int>(dimensions.x * dimensions.y * dimensions.z * 4);
            var colors = new List<Color>(dimensions.x * dimensions.y * dimensions.z);
            ...
            foreach (var path in paths)
            {
                ...
                Color[] imagePixels = texture.GetPixels(); // Order priority: X -> Y -> Z
                colors.AddRange(imagePixels);
            }
            Texture2D.DestroyImmediate(texture);
        ...
        /// <param name="dimensions">The XYZ dimensions of the volume.</param>
        /// <returns>The wrapped volume data.</returns>
        private VolumeDataset FillVolumeDatasetRGB(int[] data, Color[] data_rgb, Vector3Int dimensions)
        {
            string name = Path.GetFileName(directoryPath);
